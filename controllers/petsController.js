...
const removeImage = async (req, res) => {
  ...
  const create = async (req, res) => { 
    const { pet_name, pet_age, animal_type, genre, size, description } = req.body
    ...
    await Pets.create([users_id, user_email, pet_name, pet_age, animal_type, genre, size, description])
    res.send({
      success: true,
      data : req.body
    })
...
const getByOngId = async (req, res) => {
    ...
    const current = req.params.id
    const pets = await Pets.findByOngIdPaginated({currentPage: current}, ongid)
    res.send(pets)
}
